function [sol, it_hist, ierr, x_hist] = nsold(x,f,S,tol,parms)
...
fprintf('Starting Newton Method of Kelley (2003) modified by (BP 31 oct 2017):\n')
...
% Definitions for numjac
F = @(T,Y) feval(f,Y) ;
thresh = mean(x)*1e-10*ones(size(x)) ;
FAC = [] ;
G = [] ;
% main iteration loop
...
while(fnrm > stop_tol & itc < maxit)
  ...
  if(itc == 1 | rat > rsham | itsham == 0 | armflag == 1)
    ...
    [jac,FAC,G] = numjac(F,0,x,f0,thresh,FAC,1,S,G) ;
    fprintf('Done. Factorizing it... ')
    jacf = linfactor(jac) ;
    fprintf('Done')
  end
  itsham = itsham-1;
